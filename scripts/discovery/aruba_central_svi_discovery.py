...
class ArubaCentralSviDiscovery(BaseTask):
    name = "aruba_central_svi_discovery"
    description = "Import discovered subnets and VLAN interface addresses from Aruba Central"
    category = "discovery"
    dependencies = ["IPAM"]
    default_schedule = "0 4 * * 1"  # Weekly on Monday at 4 AM
    max_runtime = 2400
    
    def __init__(self):
        super().__init__()
        self.git_dir = "/app/git-repos/aruba-central-discovery"
        self.seed_dir = f"{self.git_dir}/seed-files"
        self.hash_file = "/var/automation_file/discovery/aruba_central_hash.txt"
        self.output_dir = "/var/automation_file/discovery"
        
    def execute(self):
        try:
            self.log("Starting Aruba Central SVI discovery")
            ...
            self.log(f"Aruba Central SVI discovery failed: {e}", level="ERROR")
            return {"status": "error", "error": str(e)}
    
    def _get_discovery_files(self):
        """Get list of Aruba Central discovery files"""
        ...
        return [
            ...
            if entry.is_file() and entry.name.endswith("vlans.csv")
        ]
    ...
    def _process_network_data(self, network_data):
        ...
        try:
            ...
            network_address = network_data.get('network_address')
    ...
    def _should_import_network(self, network_data):
        ...
        try:
            ...
            exclusions = [
                ...
                '10.0.0.0/8'      # Private networks (if configured to exclude)
            ]
    ...
    def _import_network_to_ipam(self, network_data):
        ...
        try:
            ...
            import_payload = {
                ...
                "vlan_id": network_data.get('vlan_id'),
                "description": f"Auto-discovered from {network_data['source_device']}",
    ...
    def _generate_discovery_report(self, discovered_subnets):
        ...
        try:
            ...
            report_file = f"{self.output_dir}/aruba_central_discovery_{timestamp}.csv"
...
task_class = ArubaCentralSviDiscovery
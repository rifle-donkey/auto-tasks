...
class CiscoAnsibleSviDiscovery(BaseTask):
    name = "cisco_ansible_svi_discovery"
    description = "Import discovered subnets and VLAN interface addresses from Cisco Ansible reports"
    category = "discovery"
    dependencies = ["IPAM"]
    default_schedule = "0 4 * * 2"  # Weekly on Tuesday at 4 AM
    max_runtime = 2400
    
    def __init__(self):
        super().__init__()
        self.git_dir = "/app/git-repos/cisco-ansible-discovery"
        self.seed_dir = f"{self.git_dir}/seed-files"
        self.hash_file = "/var/automation_file/discovery/cisco_ansible_hash.txt"
        self.output_dir = "/var/automation_file/discovery"
        
    def execute(self):
        try:
            self.log("Starting Cisco Ansible SVI discovery")
            ...
            self.log(f"Cisco Ansible SVI discovery failed: {e}", level="ERROR")
            return {"status": "error", "error": str(e)}
    
    def _get_discovery_files(self):
        """Get list of Cisco Ansible discovery files"""
        ...
        return [
            ...
            if entry.is_file() and ("interface" in entry.name.lower() or "svi" in entry.name.lower()) and entry.name.endswith(".csv")
        ]
    ...
    def _process_network_data(self, network_data):
        ...
        try:
            # Extract network information (Cisco format)
            interface_ip = network_data.get('interface_ip')
            subnet_mask = network_data.get('subnet_mask')
            vlan_id = network_data.get('vlan_id')
            device_name = network_data.get('hostname')
            interface_name = network_data.get('interface')
            
            if not interface_ip or not subnet_mask:
                return None
            
            # Calculate network from interface IP and mask
            network = ipaddress.IPv4Interface(f"{interface_ip}/{subnet_mask}").network
            ...
            processed_data = {
                ...
                'discovery_source': 'cisco_ansible'
            }
    ...
    def _import_network_to_ipam(self, network_data):
        ...
        try:
            ...
            import_payload = {
                ...
                "gateway_ip": network_data.get('gateway_ip'),
                "vlan_id": network_data.get('vlan_id'),
                "interface_name": network_data.get('interface_name'),
                "description": f"Auto-discovered SVI from {network_data['source_device']}",
    ...
    def _generate_discovery_report(self, discovered_subnets):
        ...
        try:
            ...
            report_file = f"{self.output_dir}/cisco_ansible_discovery_{timestamp}.csv"
...
task_class = CiscoAnsibleSviDiscovery
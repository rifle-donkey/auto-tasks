...
class AristaCvpHardwareRefresh(BaseTask):
    name = "arista_cvp_hardware_refresh"
    description = "Update Arista device hardware info from CVP data via Git seed files"
    category = "hardware"
    dependencies = ["IPAM"]
    default_schedule = "0 3 * * 3"  # Weekly on Wednesday at 3 AM
    max_runtime = 1800
    
    def __init__(self):
        super().__init__()
        self.git_dir = "/app/git-repos/arista-cvp-data"
        self.seed_dir = f"{self.git_dir}/seed-files"
        self.hash_file = "/var/automation_file/hardware/arista_cvp_hash.txt"
        
    def execute(self):
        try:
            self.log("Starting Arista CVP hardware refresh")
            ...
            self.log(f"Arista CVP hardware refresh failed: {e}", level="ERROR")
            return {"status": "error", "error": str(e)}
    
    def _get_seed_files(self):
        """Get list of Arista CVP seed files"""
        ...
        return [
            ...
            if entry.is_file() and entry.name.endswith("arista_devices.csv")
        ]
    ...
    def _read_device_data(self, seed_file_path):
        """Read Arista device data from seed file"""
        devices = []
        try:
    ...
    def _update_device_hardware(self, device_data):
        """Update hardware info for a single Arista device in IPAM"""
        try:
            ...
            update_payload = {
                ...
                "hardware_model": device_data.get("modelName"),
                "software_version": device_data.get("systemMacAddress"),
                "management_ip": device_data.get("ipAddress"),
                "device_status": device_data.get("streamingStatus"),
...
task_class = AristaCvpHardwareRefresh
...
class ArubaCentralHardwareRefresh(BaseTask):
    name = "aruba_central_hardware_refresh"
    description = "Update Aruba device hardware info from Aruba Central data via Git seed files"
    category = "hardware"
    dependencies = ["IPAM"]
    default_schedule = "0 3 * * 4"  # Weekly on Thursday at 3 AM
    max_runtime = 1800
    
    def __init__(self):
        super().__init__()
        self.git_dir = "/app/git-repos/aruba-central-data"
        self.seed_dir = f"{self.git_dir}/seed-files"
        self.hash_file = "/var/automation_file/hardware/aruba_central_hash.txt"
        
    def execute(self):
        try:
            self.log("Starting Aruba Central hardware refresh")
            ...
            for seed_file in seed_files:
                ...
                for device in devices:
                    try:
                        ...
                        self.log(f"Error updating device {device.get('name', 'unknown')}: {e}", level="ERROR")
            ...
            self.log(f"Aruba Central hardware refresh failed: {e}", level="ERROR")
            return {"status": "error", "error": str(e)}
    
    def _get_seed_files(self):
        """Get list of Aruba Central seed files"""
        ...
        return [
            ...
            if entry.is_file() and entry.name.endswith("aruba_devices.csv")
        ]
    ...
    def _read_device_data(self, seed_file_path):
        """Read Aruba device data from seed file"""
        devices = []
        try:
    ...
    def _update_device_hardware(self, device_data):
        """Update hardware info for a single Aruba device in IPAM"""
        try:
            ...
            update_payload = {
                ...
                "software_version": device_data.get("firmware_version"),
            ...
            if response and response.get("status_code") == 200:
                self.log(f"Updated hardware info for {device_data.get('name')}")
                return True
            else:
                self.log(f"Failed to update {device_data.get('name')}: {response}", level="ERROR")
                return False
...
task_class = ArubaCentralHardwareRefresh
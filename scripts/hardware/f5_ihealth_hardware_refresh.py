...
class F5IHealthHardwareRefresh(BaseTask):
    name = "f5_ihealth_hardware_refresh"
    description = "Update F5 device hardware info from iHealth data via Git seed files"
    category = "hardware"
    dependencies = ["IPAM"]
    default_schedule = "0 3 * * 1"  # Weekly on Monday at 3 AM
    max_runtime = 1800
    
    def __init__(self):
        super().__init__()
        self.git_dir = "/app/git-repos/f5-ihealth-data"
        self.seed_dir = f"{self.git_dir}/seed-files"
        self.hash_file = "/var/automation_file/hardware/f5_ihealth_hash.txt"
        
    def execute(self):
        try:
            self.log("Starting F5 iHealth hardware refresh")
            ...
            self.log(f"F5 hardware refresh failed: {e}", level="ERROR")
            return {"status": "error", "error": str(e)}
    
    def _get_seed_files(self):
        """Get list of F5 iHealth seed files"""
        ...
        return [
            ...
            if entry.is_file() and entry.name.endswith("ihealth-ipam.csv")
        ]
    ...
    def _read_device_data(self, seed_file_path):
        """Read F5 device data from seed file"""
        devices = []
        try:
    ...
    def _update_device_hardware(self, device_data):
        """Update hardware info for a single F5 device in IPAM"""
        try:
...
task_class = F5IHealthHardwareRefresh
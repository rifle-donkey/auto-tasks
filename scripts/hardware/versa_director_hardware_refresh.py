...
class VersaDirectorHardwareRefresh(BaseTask):
    name = "versa_director_hardware_refresh"
    description = "Update Versa device hardware info from Director data via Git seed files"
    category = "hardware"
    dependencies = ["IPAM"]
    default_schedule = "0 3 * * 5"  # Weekly on Friday at 3 AM
    max_runtime = 1800
    
    def __init__(self):
        super().__init__()
        self.git_dir = "/app/git-repos/versa-director-data"
        self.seed_dir = f"{self.git_dir}/seed-files"
        self.hash_file = "/var/automation_file/hardware/versa_director_hash.txt"
        
    def execute(self):
        try:
            self.log("Starting Versa Director hardware refresh")
            ...
            for seed_file in seed_files:
                ...
                for device in devices:
                    try:
                        ...
                        self.log(f"Error updating device {device.get('device-name', 'unknown')}: {e}", level="ERROR")
            ...
            self.log(f"Versa Director hardware refresh failed: {e}", level="ERROR")
            return {"status": "error", "error": str(e)}
    
    def _get_seed_files(self):
        """Get list of Versa Director seed files"""
        ...
        return [
            ...
            if entry.is_file() and entry.name.endswith("versa_devices.csv")
        ]
    ...
    def _read_device_data(self, seed_file_path):
        """Read Versa device data from seed file"""
        devices = []
        try:
    ...
    def _update_device_hardware(self, device_data):
        """Update hardware info for a single Versa device in IPAM"""
        try:
            ...
            update_payload = {
                "hostname": device_data.get("device-name"),
                "serial_number": device_data.get("serial-number"),
                "hardware_model": device_data.get("device-type"),
                "software_version": device_data.get("software-version"),
                "management_ip": device_data.get("mgmt-ip"),
                "deployment_mode": device_data.get("deployment-mode"),
            ...
            if response and response.get("status_code") == 200:
                self.log(f"Updated hardware info for {device_data.get('device-name')}")
                return True
            else:
                self.log(f"Failed to update {device_data.get('device-name')}: {response}", level="ERROR")
                return False
...
task_class = VersaDirectorHardwareRefresh